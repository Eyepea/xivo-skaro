...
class CTIServer:
    ...
    def __init__(self):
        ...
        persister = alarm.JSONPersister(config.ALARM_DIRECTORY)
    ...
    def find_matching_ipbxid(self, ipaddress):
        found_ipbxid = None
        for ipbxid, ipbxconfig in config.cconf.getconfig('ipbxes').iteritems():
    ...
    def updates_period(self):
        return int(config.cconf.getconfig('main').get('updates_period', '3600'))
    ...
    def _init_queue_stats(self):
        queue_stats_uri = config.cconf.getconfig('main')['asterisk_queuestat_db']
    ...
    def main_loop(self):    # {
        ...
        config.cconf = cti_config.Config(config.XIVO_CONF_FILE, config.XIVO_CONF_FILE_DEFAULT)
        config.cconf.set_ipwebs(config.XIVOIP)
        if config.XIVO_CONF_OVER:
            urldata = urllib.urlopen(config.XIVO_CONF_OVER)
            payload = urldata.read()
            urldata.close()
            overconf = cjson.decode(payload)
            config.cconf.set_rcti_override_ipbxes(overconf)
        config.cconf.update()
        config.cconf.set_rcti_special_profile()

        self._init_queue_stats()

        xivoconf_general = config.cconf.getconfig('main')
        ...
        log.info("the monitored ipbx's is/are : %s" % config.cconf.getconfig('ipbxes').keys())
        ...
        if self.myipbxid:
            ipbxconfig = config.cconf.getconfig('ipbxes').get(self.myipbxid)
        ...
        for kind, bind_and_port in xivoconf_general.get('incoming_tcp', {}).iteritems():
            ...
            try:
                ...
                trueport = int(port) + config.PORTDELTA
...
##                log.info('loop_over_cti_queue %s %d items' % (ipbxid, queuesize))
    ...
    def _schedule_alarms(self):
        ...
        global_zone = config.cconf.getconfig('ipbxes').get(self.myipbxid, {}).get('timezone')
                ...
                for sel_i in sels_i: # {
                    ...
                    if sel_i in self.fdlist_ami.keys():
                        try:
                            ...
                            buf = sel_i.recv(config.BUFSIZE_LARGE)
                            if len(buf) == 0:
                    ...
                    # } the UDP messages (ANNOUNCE) are catched here
                        ...
                        if kind == 'ANNOUNCE':
                            [data, sockparams] = sel_i.recvfrom(config.BUFSIZE_LARGE)
                    ...
                    # } the new TCP connections (CTI, WEBI, FAGI, INFO) are catched here
                        ...
                        if kind == 'CTI':
                            ...
                            try:
                                connstream = ssl.wrap_socket(connc,
                                                             server_side = True,
                                                             certfile = config.cconf.getconfig('certfile'),
                                                             keyfile = config.cconf.getconfig('keyfile'),
                                                             ssl_version = config.SSLPROTO)
                                connc = ClientConnection(connstream, sockparams, ctiseparator)
                            except ssl.SSLError:
                                log.exception('%s:%s:%d cert=%s key=%s)'
                                              % (kind, sockparams[0], sockparams[1],
                                                 config.cconf.getconfig('certfile'),
                                                 config.cconf.getconfig('keyfile')))
                                connc.close()
                                connc = None
                        ...
                        if connc:
                            ...
                            if kind in ['CTI', 'CTIS']:
                                logintimeout = int(config.cconf.getconfig('main').get('logintimeout', 5))
                                # logintimeout = 3600
                    ...
                    # } incoming TCP connections (CTI, WEBI, AGI, INFO)
                        ...
                        try: # {
                            ...
                            if isinstance(sel_i, ClientConnection):
                                ...
                                try:
                                    msg = sel_i.recv(config.BUFSIZE_LARGE, socket.MSG_DONTWAIT)
                                    lmsg = len(msg)
                                except Exception:
