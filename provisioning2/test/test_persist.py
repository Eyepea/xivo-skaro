...
import unittest
import prov2.persist.memory as memory
from prov2.persist.common import ID_KEY
from prov2.persist.id import numeric_id_generator, urandom_id_generator
from prov2.persist.util import _select_value, _create_predicate_from_selector,\
...
class TestSelectorSelectValue(unittest.TestCase):
    def test_select_value_simple(self):
        doc = {'k': 'v'}
        self.assertEqual(['v'], list(_select_value('k', doc)))
    ...
    def test_select_value_simple_no_match(self):
        doc = {}
        self.assertEqual([], list(_select_value('k', doc)))
    
    def test_select_value_dict(self):
        doc = {'k': {'kk': 'v'}}
        self.assertEqual(['v'], list(_select_value('k.kk', doc)))
    
    def test_select_value_dict_3depth(self):
        doc = {'k': {'kk': {'kkk': 'v'}}}
        self.assertEqual(['v'], list(_select_value('k.kk.kkk', doc)))
    
    def test_select_value_list(self):
        doc = {'k': ['v1', 'v2']}
        self.assertEqual(['v1', 'v2'],
                         list(_select_value('k', doc)))
...
class TestSelectorCreatePredicate(unittest.TestCase):
    ...
    def test_simple_2item_selector_match(self):
        pred = _create_predicate_from_selector({'k1': 'v1', 'k2': 'v2'})
        self.assertTrue(pred({'k1': 'v1', 'k2': 'v2'}))
    ...
    def test_1item_dict_selector_match(self):
        pred = _create_predicate_from_selector({'k.kk': 'v'})
