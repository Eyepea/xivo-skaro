...
function xivo_ast_meetme_http_search_user(dwsptr) {
	new dwho.http('/service/ipbx/ui.php/pbx_settings/meetme/users/search/?'
			+ dwho_sess_str, {
...
var xivo_ast_meetme_suggest_user = new dwho.suggest({
	'requestor' : xivo_ast_meetme_http_search_user
});
...
var xivo_ast_fm_meetme_admin_typefrom_elt = {
	...
	'fd-meetme-admin-suggest' : {
		style : {
			display : 'none'
		}
	},
	'it-meetme-admin-suggest' : {
		property : {
			disabled : true
		}
	},
	'fd-meetmefeatures-admin-externalid' : {
		style : {
			display : 'none'
		}
	},
	'it-meetmefeatures-admin-externalid' : {
		property : {
			disabled : true
		}
	},
	...
	'it-meetmefeatures-admin-initiallymuted' : {
		property : {
			disabled : false,
			className : 'it-enabled'
		}
	...
	'it-meetmefeatures-admin-quiet' : {
		property : {
			disabled : false,
			className : 'it-enabled'
		}
	},
	'it-meetmefeatures-admin-starmenu' : {
		property : {
			disabled : false,
			className : 'it-enabled'
		}
	},
	'it-meetmefeatures-admin-closeconflastmarkedexit' : {
		property : {
			disabled : false,
			className : 'it-enabled'
		}
...
xivo_ast_fm_meetme_admin_typefrom['none']['it-meetmefeatures-admin-initiallymuted']['property'] = {
	disabled : true,
	className : 'it-disabled'
};
...
xivo_ast_fm_meetme_admin_typefrom['none']['it-meetmefeatures-admin-quiet']['property'] = {
	disabled : true,
	className : 'it-disabled'
};
xivo_ast_fm_meetme_admin_typefrom['none']['it-meetmefeatures-admin-starmenu']['property'] = {
	disabled : true,
	className : 'it-disabled'
};
xivo_ast_fm_meetme_admin_typefrom['none']['it-meetmefeatures-admin-closeconflastmarkedexit']['property'] = {
	disabled : true,
	className : 'it-disabled'
};
xivo_ast_fm_meetme_admin_typefrom['none']['it-meetmefeatures-admin-enableexitcontext']['property'] = {
	disabled : true,
	className : 'it-disabled'
};
...
xivo_ast_fm_meetme_admin_typefrom['internal']['it-meetmefeatures-admin-internalid']['property'] = {
	disabled : false
};
xivo_ast_fm_meetme_admin_typefrom['internal']['fd-meetme-admin-suggest']['style'] = {
	display : 'block'
};
xivo_ast_fm_meetme_admin_typefrom['internal']['it-meetme-admin-suggest']['property'] = {
	disabled : false
};
...
xivo_ast_fm_meetme_admin_typefrom['external']['fd-meetmefeatures-admin-externalid']['style'] = {
	display : 'block'
};
xivo_ast_fm_meetme_admin_typefrom['external']['it-meetmefeatures-admin-externalid']['property'] = {
	disabled : false
};
...
function xivo_ast_meetme_chg_admin_moderationmode() {
	if ((moderationmode = dwho_eid('it-meetmefeatures-admin-moderationmode')) === false)
		...
		dwho.form
				.reset_field(dwho_eid('it-meetmefeatures-admin-announcejoinleave'));
		dwho.form
				.reset_field(dwho_eid('it-meetmefeatures-user-announcejoinleave'));
	}
...
function xivo_ast_meetme_suggest_event_admin() {
	xivo_ast_meetme_suggest_user.set_option('result_field',
			'it-meetmefeatures-admin-internalid');
	xivo_ast_meetme_suggest_user.set_option('result_onclearfield', function() {
		dwho.form.set_text_helper('it-meetme-admin-suggest', true);
	});
...
//get available extensions
function map_autocomplete_extension(obj){
	context = $('#it-meetmefeatures-context').val();
	$.getJSON('/service/ipbx/ui.php/pbx_settings/extension/search/?obj=meetme&format=jquery&context='+context, function(data) {
		if (data === null || (nb = data.length) === 0)
			return false;
		$('#it-meetmefeatures-confno').autocomplete({
			source: data.split('\n')
		});
	});
}
...
function xivo_ast_meetme_onload() {
	...
	$('#it-meetmefeatures-confno').live({
			focus: function() {
				$('#helper-context_num_pool').show('slow');
				map_autocomplete_extension();
			},
			blur: function() {
				$('#helper-context_num_pool').hide('slow');
			}
	});

	dwho.dom.add_event('change', dwho_eid('it-meetmefeatures-admin-typefrom'),
	...
	dwho.dom.add_event('focus', dwho_eid('it-meetme-admin-suggest'),
