...
function xivo_genercache() {
	...
	function make(idtype, data) {
		...
		var remaining_time = Math
				.round((average(avg) * (total - counter)) / 1000);

		infos = '';
		infos += '<p>';
		infos += '<b>' + humandate + '</b> ' + jstxt_in_progress;
		infos += '........... ';
		infos += jstxt_process_last_time_traitment + ' '
				+ Math.round(diff2 / 1000) + 's';
		infos += '</p>';
		...
		if (idtype == 'all') {
			...
			// $('#restitle-all').html(object_processing + ' ' +
			// objectProcessing);
			$(function() {
				$('#resprogressbar-all').progressbar({
					value : pct
				});
			});
		} else {
			...
			// $('#restitle-'+idtyp).html(object_processing + ' ' +
			// objectProcessing);
			$(function() {
				$('#resprogressbar-' + idtype).progressbar({
					value : pct
				});
			});
		}

		request_post(jsvar_idconf, listmonthfirstday[counter],
				listmonthlastday[counter], jsvar_type, idtype);
	...
	function request_post(idconf, dbeg, dend, type, idtype) {
		$.ajax({
			type : 'POST',
			url : '/statistics/call_center/ui.php/settings/genercache/',
			data : {
				'idconf' : idconf,
				'dbeg' : dbeg,
				'dend' : dend,
