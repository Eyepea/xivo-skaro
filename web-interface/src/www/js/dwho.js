...
function dwho_has_len(val, key) {
	if (dwho_is_undef(key) === false && dwho_type_object(val) === true
...
function dwho_long2ip(value) {
	...
	var r = ((value >> 24) & 0xFF) + '.' + ((value >> 16) & 0xFF) + '.'
...
function dwho_chk_host(value) {
	if (dwho_is_string(value) === false || value.length < 4
			|| value.length > 255)
		return (false);
...
function dwho_chk_ipv4_subnet(value) {
	if (dwho_is_string(value) === false || (pos = value.indexOf('/')) < 7)
		return (false);
...
function dwho_chk_ipv4_netmask_bit(bit) {
	if (dwho_is_uint(bit) === false || bit > 32)
		return (false);
...
function dwho_chk_ipv4_netmask_dotdec(nm) {
	if ((nm = dwho_ip2long(nm)) === 0)
		...
		return (32);
	else if (nm === false || (inv = (nm ^ 0xFFFFFFFF)) === 0
			|| (inv & (inv + 1)) !== 0)
		return (false);
...
function dwho_strcmp(str1, str2, len) {
	if (dwho_is_scalar(str1) === false || dwho_is_scalar(str2) === false)
		return (false);
