...
function xivo_bwcheck() {
	...
	this.cookie = typeof (navigator.cookieEnabled) !== 'undefined' ? navigator.cookieEnabled
			: false;
...
function xivo_open_center(url, name, width, height, param) {
	...
	return open(url, name, 'left=' + cx + 'px,top=' + cy + 'px,width=' + width
			+ 'px,height=' + height + 'px' + param);
}
...
function xivo_chg_attrib(name, id, type, link) {
	...
	for ( var property in ref_elem['link']) {
		...
		xivo_chg_attrib(name, ref_elem['link'][property][0],
				ref_elem['link'][property][1], nlink);
	}
}

function xivo_menu_active() {
	var xivo_menu_active = 'mn-'
			+ dwho_location_action_path.replace(/\//g, '--');
...
function xivo_form_select_add_host_ipv4_subnet(id, value) {
	if (dwho_chk_ipv4_strict(value) === false && dwho_chk_host(value) === false
