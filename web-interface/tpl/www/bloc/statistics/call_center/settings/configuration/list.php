...
?>
<form action="#" name="fm-conf-list" method="post" accept-charset="utf-8">
<?php
...
<?php
	if(($list = $this->get_var('list')) === false
	|| ($nb = count($list)) === 0):
?>
	<tr class="sb-content">
		<td colspan="4" class="td-single"><?=$this->bbf('no_conf');?></td>
	</tr>
<?php
	else:
		for($i = 0;$i < $nb;$i++):
...
?>
	<tr onmouseover="this.tmp = this.className; this.className = 'sb-content l-infos-over';"
		...
		<td class="td-left">
			<?=$form->checkbox(array('name'		=> 'stats_conf[]',
						 'value'	=> $ref['id'],
						 'label'	=> false,
						 'id'		=> 'it-conf-'.$i,
						 'checked'	=> false,
...
<?php
				echo	$url->img_html('img/site/flag/'.$icon.'.gif',null,'class="icons-list"'),
					dwho_htmlen(dwho_trunc($ref['name'],40,'...',false));
?>
			</label>
		</td>
