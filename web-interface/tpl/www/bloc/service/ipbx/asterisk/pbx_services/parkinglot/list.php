...
<table id="table-main-listing" cellspacing="0" cellpadding="0" border="0">
	<tr class="sb-top">
		...
		<th class="th-center"><?=$this->bbf('col_extension');?></th>
		<th class="th-center"><?=$this->bbf('col_positions');?></th>
...
?>
	<tr class="sb-content">
		<td colspan="7" class="td-single"><?=$this->bbf('no_parkinglot');?></td>
	</tr>
<?php
	else:
		for($i = 0;$i < $nb;$i++):
			...
			if($ref['commented'] === true || $ref['commented'] == 1):
				$icon = 'disable';
			else:
				$icon = 'enable';
	endif;
...
?>
	<tr onmouseover="this.tmp = this.className; this.className = 'sb-content l-infos-over';"
		...
		<td class="td-left">
			<?=$form->checkbox(array('name'		=> 'parkinglots[]',
						 'value'	=> $ref['id'],
						 'label'	=> false,
						 'id'		=> 'it-parkinglots-'.$i,
						 'checked'	=> false,
		...
		<td class="txt-left">
			<label for="it-parkinglots-<?=$i?>" id="lb-parkinglots-<?=$i?>">
<?php
