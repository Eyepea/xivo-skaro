...
?>
<form action="#" name="fm-operator-list" method="post" accept-charset="utf-8">
<?php
...
?>
		...
		<th class="th-center"><?= $this->bbf('col_currency');?></th>
...
?>
	<tr class="sb-content">
		<td colspan="5" class="td-single"><?=$this->bbf('no_operator');?></td>
	</tr>
<?php
	else:
		for($i = 0;$i < $nb;$i++):
...
?>
	<tr onmouseover="this.tmp = this.className; this.className = 'sb-content l-infos-over';"
		...
		<td class="td-left">
			<?=$form->checkbox(array('name'		=> 'operator[]',
						 'value'	=> $ref['id'],
						 'label'	=> false,
						 'id'		=> 'it-operator-'.$i,
						 'checked'	=> false,
		...
		<td class="txt-left" title="<?=dwho_alttitle($ref['name']);?>">
			<label for="it-operator-<?=$i?>" id="lb-operator-<?=$i?>">
<?php
...
?>
		...
		<td><?=dwho_htmlen(dwho_trunc($ref['currency'],25,'...',false));?></td>
