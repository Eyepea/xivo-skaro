...
$error = false;
$param['act'] = 'list';
$info = array('agents' => array(), 'queues' => array(), 'filters' => array('skills' => ''));
	...
	case 'add':
		...
		&& dwho_issa('campaign', $_QR) === true)
		{
			...
			&& ($id = $campmod->add($campaign)) !== false)
			{
				...
				$_QRY->go($_TPL->url('callcenter/registrationcampaign/campaigns'), $param);
				break;
			}
		...
		{
			if(array_key_exists('agents', $_QR) && in_array($agent['agentfeatures']['id'], $_QR['agents']))
		...
		{
			if(array_key_exists('queues', $_QR) && in_array($queue['id'], $_QR['queues']))
...
$dhtml->set_js('js/callcenter/registrationcampaign/campaigns.js');
...
$menu->set_toolbar('toolbar/callcenter/registrationcampaign/campaigns');

$_TPL->set_bloc('main','callcenter/registrationcampaign/campaigns/'.$act);
